...
const backupEngine = fs.readFileSync('lib/pocketstomp-backup.mjs', 'utf8');
...
expect('no destructive storage clear', !page.includes('localStorage.clear') && !backupEngine.includes('localStorage.clear'));
...
expect('backup engine imported into recovered V2 UI', page.includes('../lib/pocketstomp-backup.mjs'));
expect('user backup button exists', page.includes('BACK UP POCKETSTOMP DATA'));
expect('merge restore button exists', page.includes('RESTORE / MERGE BACKUP'));
expect('JSON backup input is constrained', page.includes('accept="application/json,.json"'));
expect('pre-import safety backup is generated before restore write', page.indexOf('pocketstomp-pre-import') >= 0 && page.indexOf('pocketstomp-pre-import') < page.indexOf('mergePocketStompBackupIntoStorage(localStorage, backupState)'));
expect('backup safety limit wired into UI', page.includes('POCKETSTOMP_BACKUP.maxBytes'));
expect('backup engine schema marker', backupEngine.includes("schema: 'pocketstomp-backup-v1'"));
expect('backup engine strips prototype-pollution keys', backupEngine.includes("'__proto__'") && backupEngine.includes("'constructor'") && backupEngine.includes("'prototype'"));
expect('backup engine transactional rollback exists', backupEngine.includes('rolled back') && backupEngine.includes('rollback'));
